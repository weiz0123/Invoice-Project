...
class ExtendedUi_MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):  # QtWidgets.QWidget
    def __init__(self, path):
        super().__init__()
        self.company_path = path + "\Company"
        self.image_folder_path = ""       # first initialized in init_label
        self.count_resize = 1
    ...
    def init_label(self):
        ...
        self.image_folder_path = self.company_path + "\\" + company + "\Images"
        entries = self.get_entries(self.image_folder_path)
        first_entry = entries[0]
        for entry in entries:
            if entry.fileName() not in ('.', '..'):
                first_entry = entry
                break
        image_path = self.image_folder_path + "\\" + first_entry.fileName()
    ...
    def initialize_file_explorer(self):
        ...
        self.ui.File_Explore_List_Widget.populate_tree(self.company_path)
    ...
    def update_company_combo_box(self):
        entries = self.get_entries(self.company_path)
        for entry in entries:
    ...
    def update_status(self):
        ...
        try:
            ...
            company_entries = self.get_entries(self.image_folder_path)
            num_inv = 0
    ...
    def open_company_folder(self):

        print(self.company_path)
        try:
            """only supported for window now"""
            # Use the appropriate command based on the operating system
            if sys.platform.startswith('win'):
                subprocess.Popen(['explorer', self.company_path], shell=True)
            elif sys.platform.startswith('darwin'):
                subprocess.Popen(['open', self.company_path])
            elif sys.platform.startswith('linux'):
                subprocess.Popen(['xdg-open', self.company_path])
            else:
